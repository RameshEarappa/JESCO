report 50106 "Jesco Sales - Order Conf."
{
    RDLCLayout = '.\Layout Reports Rdls\JescoSalesOrderConf.rdl';
    ...
    dataset
    {
        dataitem(Header; "Sales Header")
        {
            ...
            column(Quote_Valid_Until_Date; ValidUptoG) //"Quote Valid Until Date")
            {
            }
            ...
            column(CompanyPicture; CompanyInfo.Picture) //DummyCompanyInfo.Picture)
            {
            }
            ...
            column(ShipmentMethodDescription; TransportMethod.Description)//"Transport Method")//ShipmentMethod.Description)
            {
            }
            ...
            dataitem(Line; "Sales Line")
            {
                ...
                column(UnitPrice; UnitPriceG)//FormattedUnitPrice)
                {
                    AutoFormatExpression = "Currency Code";
                    AutoFormatType = 2;
                }
                ...
                trigger OnAfterGetRecord()
                begin
                    if Type IN [Type::Item, Type::"G/L Account", Type::"Fixed Asset", Type::Resource] then
                        SLNo += 1;
                    ...
                    Clear(UnitPriceG);
                    if (Quantity <> 0) AND (GetLineAmountExclVAT() <> 0) then
                        UnitPriceG := GetLineAmountExclVAT() / Quantity;
    ...
    var
        ...
        GLDiscountAmtTxt: Label 'Inv. Discount Amount';
